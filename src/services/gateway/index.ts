...
import {
  ...
  type GatewayConfig,
} from '@/services/gateway/config.js';
import { loginWhatsApp } from '@/services/gateway/channels/whatsapp/login.js';
import { startGateway } from '@/services/gateway/gateway.js';
...
async function promptSetupMode(cfg: GatewayConfig, linkedPhone: string): Promise<void> {
  ...
  try {
    ...
    console.log('');
    console.log('How will you use Sentinel with WhatsApp?');
    console.log('  1) Self-chat  — message yourself to talk to Sentinel');
    console.log('  2) Bot phone  — this is a dedicated Sentinel phone, others message it');
    ...
    console.log('');
    console.log('Enter the phone number(s) allowed to message Sentinel (E.164 format, e.g. +15551234567).');
...
async function run(): Promise<void> {
  ...
  console.log('Sentinel gateway running. Press Ctrl+C to stop.');
