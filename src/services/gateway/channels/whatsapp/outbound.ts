...
import type { WaSocket } from '@/services/gateway/channels/whatsapp/session.js';
import { loadGatewayConfig, resolveWhatsAppAccount } from '@/services/gateway/config.js';
import { normalizeE164, toWhatsappJid } from '@/services/gateway/utils.js';
import { sentinelPath } from '@/common/utils/paths.js';

function debugLog(msg: string) {
  try {
    const logDir = sentinelPath('debug', 'logs');
    const logPath = sentinelPath('debug', 'logs', 'gateway-outbound.log');
...
function getActive(accountId?: string): ActiveListener {
  ...
  if (!first) {
    throw new Error('No active WhatsApp listener. Run sentinel gateway run.');
  }
  return first;
}
