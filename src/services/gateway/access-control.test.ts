...
import { checkInboundAccessControl, isAllowedPhone, recordPairingRequest } from '@/services/gateway/access-control.js';
...
describe('access control', () => {
  ...
  test('records pairing request for unknown sender', () => {
    const dir = mkdtempSync(join(tmpdir(), 'sentinel-pairing-'));
    const path = join(dir, 'whatsapp.json');
    process.env.SENTINEL_PAIRING_PATH = path;
    try {
      ...
      delete process.env.SENTINEL_PAIRING_PATH;
