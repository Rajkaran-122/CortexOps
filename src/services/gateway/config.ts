...
import { z } from 'zod';
import { normalizeE164 } from '@/services/gateway/utils.js';
import { sentinelPath } from '@/common/utils/paths.js';

const DEFAULT_GATEWAY_PATH = sentinelPath('gateway.json');
...
export function getGatewayConfigPath(overridePath?: string): string {
  return overridePath ?? process.env.SENTINEL_GATEWAY_CONFIG ?? DEFAULT_GATEWAY_PATH;
}
...
export function resolveWhatsAppAccount(
  ...
  const authDir = account.authDir ?? sentinelPath('credentials', 'whatsapp', accountId);
