...
import {
  ...
  upsertSessionMeta,
} from '@/services/gateway/sessions/store.js';

describe('session store', () => {
  test('creates and updates session metadata', () => {
    const dir = mkdtempSync(join(tmpdir(), 'sentinel-sessions-'));
    process.env.SENTINEL_SESSIONS_DIR = dir;
    try {
      ...
      delete process.env.SENTINEL_SESSIONS_DIR;
