...
import type { TokenUsage } from '@/core/orchestrator/types.js';
import { theme } from '@/interface/theme.js';
import { AnswerBoxComponent } from '@/interface/components/answer-box.js';
import { ToolEventComponent } from '@/interface/components/tool-event.js';
import { UserQueryComponent } from '@/interface/components/user-query.js';
...
export class ChatLogComponent extends Container {
  ...
  addInterrupted() {
    this.addChild(new Text(`${theme.muted('⎿  Interrupted · What should Sentinel do instead?')}`, 0, 0));
  }
