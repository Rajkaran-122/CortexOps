/**
 * LangSmith Evaluation Runner for Sentinel
 * 
 * Usage:
...
import { EvalApp, type EvalProgressEvent } from '@/evals/components/index.js';
...
// ============================================================================
// Target function - wraps Sentinel agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `sentinel-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'sentinel-finance-eval';
    ...
    const experimentName = `sentinel-eval-${Date.now().toString(36)}`;
