...
import { loadGatewayConfig, saveGatewayConfig } from '@/services/gateway/config.js';
import { buildHeartbeatQuery } from '@/services/gateway/heartbeat/prompt.js';
import { loadCronStore, saveCronStore } from '@/services/cron/store.js';

const HEARTBEAT_MD_PATH = sentinelPath('HEARTBEAT.md');
const HEARTBEAT_JOB_NAME = 'Heartbeat';

export const HEARTBEAT_TOOL_DESCRIPTION = `
Manage your periodic heartbeat checklist (.sentinel/HEARTBEAT.md).
...
export const heartbeatTool = new DynamicStructuredTool({
  name: 'heartbeat',
  description:
    'View or update the heartbeat checklist (.sentinel/HEARTBEAT.md) that controls periodic monitoring.',
